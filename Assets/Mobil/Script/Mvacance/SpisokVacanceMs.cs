...
public class SpisokVacanceMs : MonoBehaviour {
   ...
   #region HASH KOD ----
    ...
    public IEnumerator GetJson(string face,System.Action<TestItemModel[]> callback){
        ...
        using (UnityWebRequest www = UnityWebRequest.Post("https://playklin.000webhostapp.com/yk/GetALLykvac.php",form)){
    ...
    void InitializeItemView (GameObject viewGameObject, TestItemModel model)
    {
        ...
        //        StartCoroutine(DeletVac(view.id.text));
    ...
    IEnumerator DeletVac(string id_servic) {
        WWWForm form = new WWWForm();
        form.AddField("id_servic", id_servic);
        //form.AddField("_text", text);
        UnityWebRequest www = UnityWebRequest.Post("https://playklin.000webhostapp.com/yk/DeletVac.php", form);
